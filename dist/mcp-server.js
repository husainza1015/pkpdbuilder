...
// ── Data Tools ──────────────────────────────────────────────
server.tool('load_dataset', 'Load a PK/PD dataset (CSV, XPT, SAS7BDAT). Returns summary statistics and column info.', { path: z.string().describe('Path to the dataset file') }, mcpWrap(dataTools.loadDataset));
server.tool('summarize_dataset', 'Get detailed summary of a loaded dataset: column types, missing values, distributions.', { path: z.string().describe('Path to the dataset file') }, mcpWrap(dataTools.summarizeDataset));
server.tool('plot_data', 'Generate exploratory PK/PD plots (concentration-time, dose-response).', {
...
server.tool('dataset_qc', 'Run quality checks on a NONMEM-format dataset: required columns, BLQ handling, dosing records.', { path: z.string().describe('Path to the dataset file') }, mcpWrap(dataTools.datasetQC));
server.tool('handle_blq', 'Handle Below Limit of Quantification (BLQ) data using M1-M7 methods.', {
    path: z.string().describe('Path to the dataset file'),
    method: z.string().optional().describe('BLQ method: M1 (discard), M3 (likelihood), M5 (LOQ/2)'),
}, mcpWrap(dataTools.handleBLQ));
// ── Modeling Tools ──────────────────────────────────────────
server.tool('fit_model', 'Fit a population PK model using nlmixr2. Supports 1/2/3-CMT, oral/IV, with IIV on CL/V.', {
...
server.tool('fit_from_library', 'Fit a model from the built-in library of published PopPK models.', {
    model_id: z.string().describe('Model ID from the library'),
    dataset_path: z.string().describe('Path to the dataset'),
    output_dir: z.string().optional().describe('Output directory'),
}, mcpWrap(modelingTools.fitFromLibrary));
server.tool('compare_models', 'Compare fitted models using OFV, AIC, BIC, and diagnostic criteria.', {
    model_paths: z.array(z.string()).describe('Paths to model result files'),
    output_dir: z.string().optional().describe('Output directory'),
}, mcpWrap(modelingTools.compareModels));
server.tool('list_model_library', 'List available models in the built-in pharmacometrics model library.', {
    category: z.string().optional().describe('Filter by category: pk, pkpd, tmdd, pbpk'),
}, mcpWrap(modelingTools.listModelLibrary));
server.tool('get_model_code', 'Get nlmixr2/NONMEM/Monolix code for a model from the library.', {
...
// ── Diagnostics Tools ───────────────────────────────────────
server.tool('goodness_of_fit', 'Generate Goodness-of-Fit (GOF) diagnostic plots: DV vs PRED, DV vs IPRED, CWRES vs TIME.', {
    model_path: z.string().describe('Path to fitted model results'),
    output_dir: z.string().optional().describe('Output directory'),
}, mcpWrap(diagnosticsTools.goodnessOfFit));
server.tool('vpc', 'Generate Visual Predictive Check (VPC) plot.', {
    model_path: z.string().describe('Path to fitted model results'),
    n_sim: z.number().optional().describe('Number of simulations (default 200)'),
    output_dir: z.string().optional().describe('Output directory'),
}, mcpWrap(diagnosticsTools.vpc));
server.tool('eta_plots', 'Generate ETA distribution plots and ETA-covariate correlations.', {
    model_path: z.string().describe('Path to fitted model results'),
    output_dir: z.string().optional().describe('Output directory'),
}, mcpWrap(diagnosticsTools.etaPlots));
server.tool('individual_fits', 'Generate individual-level fit plots (observed vs predicted per subject).', {
    model_path: z.string().describe('Path to fitted model results'),
    subjects: z.array(z.number()).optional().describe('Subject IDs to plot (default: first 9)'),
    output_dir: z.string().optional().describe('Output directory'),
}, mcpWrap(diagnosticsTools.individualFits));
server.tool('parameter_table', 'Generate a formatted parameter table (estimates, RSE, IIV, shrinkage).', {
    model_path: z.string().describe('Path to fitted model results'),
    format: z.string().optional().describe('Output format: text (default), html, latex'),
}, mcpWrap(diagnosticsTools.parameterTable));
// ── NCA Tools ───────────────────────────────────────────────
server.tool('run_nca', 'Run Non-Compartmental Analysis (NCA) using PKNCA. Calculates AUC, Cmax, Tmax, t1/2, CL/F, Vd/F.', {
    dataset_path: z.string().describe('Path to the dataset'),
...
// ── Literature Tools ────────────────────────────────────────
server.tool('search_pubmed', 'Search PubMed for published PopPK/PKPD models and parameters.', {
...
server.tool('lookup_drug', 'Look up drug PK parameters from the PKPDBuilder database.', {
...
server.tool('stepwise_covariate_model', 'Run Stepwise Covariate Modeling (SCM) — forward inclusion + backward elimination.', {
    model_path: z.string().describe('Path to base model results'),
    covariates: z.array(z.string()).describe('Covariates to test'),
    output_dir: z.string().optional().describe('Output directory'),
}, mcpWrap(covariateTools.stepwiseCovariateModel));
server.tool('forest_plot', 'Generate a forest plot showing covariate effects on PK parameters.', {
    model_path: z.string().describe('Path to covariate model results'),
    output_dir: z.string().optional().describe('Output directory'),
}, mcpWrap(covariateTools.forestPlot));
// ── Export/Import Tools ─────────────────────────────────────
server.tool('export_model', 'Export a fitted model to NONMEM, Monolix, or Phoenix NLME format.', {
...
server.tool('import_model', 'Import a model from NONMEM, Monolix, or Phoenix NLME into nlmixr2.', {
...
server.tool('list_backends', 'List available modeling backends (nlmixr2, NONMEM, Monolix, Phoenix) and their status.', {}, mcpWrap(exportTools.listBackends));
// ── Report Tools ────────────────────────────────────────────
...
// ── Memory Tools ────────────────────────────────────────────
server.tool('memory_read', 'Read project memory (previous analyses, decisions, model history).', {
    project_dir: z.string().optional().describe('Project directory (default: cwd)'),
}, mcpWrap(memoryTools.memoryRead));
server.tool('memory_write', 'Write to project memory (log decisions, model selections, notes).', {
    key: z.string().describe('Memory key (e.g., "base_model", "final_model")'),
    value: z.string().describe('Value to store'),
    project_dir: z.string().optional().describe('Project directory (default: cwd)'),
}, mcpWrap(memoryTools.memoryWrite));
server.tool('memory_search', 'Search project memory for past decisions and analysis context.', {
    query: z.string().describe('Search query'),
    project_dir: z.string().optional().describe('Project directory (default: cwd)'),
}, mcpWrap(memoryTools.memorySearch));
server.tool('init_project', 'Initialize a new PKPDBuilder project directory with standard structure.', {
