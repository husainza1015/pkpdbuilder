...
export async function startRepl(): Promise<void> {
  ...
  if (discovery.r.found) {
    ...
    if (discovery.r.packages.nlmixr2) console.log(chalk.green('  ✓ nlmixr2 installed'));
    if (discovery.r.packages.mrgsolve) console.log(chalk.green('  ✓ mrgsolve installed'));
    if (!discovery.r.packages.PKNCA) {
      console.log(
        chalk.yellow('  ✗ PKNCA not found (optional — install with install.packages("PKNCA"))')
      );
    }
  } else {
  ...
  if (discovery.datasets.length > 0) {
    console.log(
      chalk.dim(`  Found ${discovery.datasets.length} dataset(s) in current directory`)
    );
  }
  ...
  while (true) {
    ...
    if (trimmed.startsWith('/')) {
      ...
      if (cmd === '/tools') {
        ...
        console.log(
          '  load_dataset, summarize_dataset, plot_data, fit_model, vpc, goodness_of_fit, run_nca, simulate_regimen, generate_report, memory_read, memory_write, ...'
        );
