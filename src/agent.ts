...
import { createProvider, Provider, Message, Tool, ChatResponse } from './providers/index.js';
...
export class PKPDBuilderAgent {
  ...
  constructor(options: AgentOptions = {}) {
    ...
    if (!apiKey && !PROVIDERS[providerName]?.local) {
      throw new Error(
        `No API key for ${providerName}. Set ${PROVIDERS[providerName]?.env_key} or run setup.`
      );
    }
