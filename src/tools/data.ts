...
let currentDatasetPath: string = '';

export const loadDatasetTool: Tool = {
  name: 'load_dataset',
  description:
    'Load a pharmacokinetic dataset from CSV or NONMEM format. Validates required columns (ID, TIME, DV). Accepts standard NONMEM columns: ID, TIME, DV, AMT, EVID, MDV, CMT, WT, AGE, SEX, etc.',
  input_schema: {
    type: 'object',
    properties: {
      file_path: {
...
export const summarizeDatasetTool: Tool = {
  name: 'summarize_dataset',
  description:
    'Generate detailed summary statistics for the currently loaded dataset. Shows subject counts, observation ranges, dose info, covariate distributions.',
  input_schema: {
    type: 'object',
    properties: {},
    required: [],
...
export async function summarizeDataset(): Promise<any> {
...
export const plotDataTool: Tool = {
  name: 'plot_data',
  description:
    'Generate exploratory plots for PK/PD data: spaghetti plots (individual profiles), scatter plots, histograms. Saves to output directory.',
  input_schema: {
    type: 'object',
    properties: {
      plot_type: {
...
export async function plotData(args: { plot_type?: string; log_y?: boolean }): Promise<any> {
  return {
    success: true,
...
export const datasetQCTool: Tool = {
  name: 'dataset_qc',
...
export async function datasetQC(args: { lloq?: number }): Promise<any> {
  return {
    success: true,
...
export const handleBLQTool: Tool = {
  name: 'handle_blq',
...
export async function handleBLQ(args: { method: string; lloq: number }): Promise<any> {
  return {
    success: true,
