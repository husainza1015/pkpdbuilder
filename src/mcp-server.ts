...
server.tool(
  'load_dataset',
  'Load a PK/PD dataset (CSV, NONMEM format). Validates required columns (ID, TIME, DV). Returns summary: rows, subjects, observations, doses.',
  {
...
server.tool(
  'summarize_dataset',
  'Get detailed summary of the currently loaded dataset: column types, distributions, subject counts.',
  {},
...
server.tool(
  'plot_data',
  'Generate exploratory PK/PD plots: spaghetti (individual profiles), scatter, histogram. Saves to output directory.',
  {
...
server.tool(
  'dataset_qc',
  'Quality checks: missing values, duplicates, outliers, LLOQ violations.',
  {
...
server.tool(
  'handle_blq',
  'Handle Below Limit of Quantification data using M1/M3/M5 methods.',
  {
...
server.tool(
  'fit_model',
  'Fit a population PK model using nlmixr2. Provide nlmixr2 model code, or describe the model and let the tool generate the code. Supports FOCEI, SAEM estimation.',
  {
...
server.tool(
  'fit_from_library',
  'Fit a pre-defined model from the built-in library (e.g., pk_1cmt_oral_1abs, pk_2cmt_iv_bolus).',
  {
...
server.tool(
  'compare_models',
  'Compare fitted models by OFV, AIC, BIC. Returns comparison table and recommendation.',
  {
...
server.tool(
  'list_model_library',
  'List all available models in the pharmacometrics model library.',
  {
...
server.tool(
  'get_model_code',
  'Get nlmixr2/NONMEM code for a model from the library.',
  {
...
server.tool(
  'goodness_of_fit',
  'Generate GOF plots: DV vs PRED, DV vs IPRED, CWRES vs TIME, CWRES vs PRED.',
  {
...
server.tool(
  'vpc',
  'Visual Predictive Check: simulate from model and compare to observed data.',
  {
    model_name: z.string().optional().describe('Model name'),
...
server.tool(
  'eta_plots',
  'ETA distribution plots and shrinkage assessment.',
  {
    model_name: z.string().optional().describe('Model name'),
  },
...
server.tool(
  'individual_fits',
  'Individual subject fit plots (observed vs predicted per subject).',
  {
    model_name: z.string().optional().describe('Model name'),
...
server.tool(
  'parameter_table',
  'Formatted parameter table with estimates, RSE, IIV, shrinkage.',
  {
    model_name: z.string().optional().describe('Model name'),
  },
...
server.tool(
  'run_nca',
  'Non-Compartmental Analysis using PKNCA: AUC, Cmax, Tmax, t½, CL/F, Vd/F.',
  {
...
server.tool(
  'search_pubmed',
  'Search PubMed for published PopPK/PD models and pharmacometric literature.',
  {
...
server.tool(
  ...
  'Stepwise Covariate Modeling (SCM): forward inclusion + backward elimination.',
  {
    model_name: z.string().optional().describe('Base model name'),
...
server.tool(
  'forest_plot',
  'Forest plot showing covariate effects on PK parameters.',
  {
...
server.tool(
  'export_model',
  'Export a fitted nlmixr2 model to NONMEM, Monolix, or Phoenix NLME format.',
  {
...
server.tool(
  'import_model',
  'Import a model from NONMEM/Monolix/Phoenix into nlmixr2.',
  {
...
server.tool(
  'list_backends',
  'List available modeling backends and their installation status.',
  {},
...
server.tool(
  'pkpd_memory_read',
  'Read PKPDBuilder project memory (previous analyses, model decisions, parameter history). Separate from CLAUDE.md — this stores structured pharmacometrics context.',
  {},
...
server.tool(
  'pkpd_memory_write',
  'Write to PKPDBuilder project memory. Use for logging model selections, parameter estimates, analysis decisions.',
  {
...
server.tool(
  'pkpd_memory_search',
  'Search PKPDBuilder project memory for past analysis decisions and context.',
  {
...
server.tool(
  'init_pkpd_project',
  'Initialize a new PKPDBuilder project: creates .pkpdbuilder/ directory with memory and output structure.',
  {
